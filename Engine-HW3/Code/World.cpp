...
void WorldManager::ConstructNormalMap(const Color4U *heightMap, Color2S *normalMap, int32 width, int32 height, float scale)
{
	...
	for (int32 y = 0; y < height; y++)
	{
		...
		for (int32 x = 0; x < width; x++)
		{
			...
			// Calculate slopes.
			float dx = (centerRow[xp1].red - centerRow[xm1].red) * 0.5F / 255.0F * scale;
			float dy = (lowerRow[x].red - upperRow[x].red) * 0.5F / 255.0F * scale;
			// Normalize and clamp.
			float nz = 1.0F / sqrt(dx * dx + dy * dy + 1.0F);
			float nx = fmin(fmax(-dx * nz, -1.0F), 1.0F);
			float ny = fmin(fmax(-dy * nz, -1.0F), 1.0F);
			normalMap[x].Set(127.0F * nx, 127.0F * ny);
		}
		normalMap += width;
	}
}
