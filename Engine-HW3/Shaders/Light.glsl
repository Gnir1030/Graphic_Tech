...
void main()
{
	...
	float2 textureNormal = texture(normalTexture, vertexTexcoord).xy;
	...
	float3 m = float3(textureNormal, sqrt(1.0 - textureNormal.x * textureNormal.x - textureNormal.y * textureNormal.y));
	...
	float3 b = cross(n, tangent) * vertexTangent.w;
	...
	float3 newNormal = tangent * m.x + b * m.y + n * m.z;
