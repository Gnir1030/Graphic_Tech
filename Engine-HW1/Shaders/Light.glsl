...
void main()
{
	...
	float3 vectorL = normalize(lightPosition.xyz - vertexPosition);

	// 3. The unit-length direction to the camera/viewer (the vector v).
	float3 vectorV = normalize(cameraPosition.xyz - vertexPosition);

	// 4. The unit-length halfway direction (the vector h).
	float3 vectorH = normalize(vectorL + vectorV);

	// 5. The squared distance between the interopolated vertex position and the light source.
	float distance = rsqrt(pow(vertexPosition.x - lightPosition.x, 2) + pow(vertexPosition.y - lightPosition.y, 2) + pow(vertexPosition.z - lightPosition.z, 2));
	...
	float expAtt = saturate(exp(dot(vectorL, vectorL) * attenConst.x) * attenConst.y - attenConst.z);
	...
	float3 fDirect = lightColor.xyz * (0.3183 * diffuseColor * saturate(dot(vertexNormal, vectorL)) + specularColor * pow(saturate(dot(vertexNormal, vectorH)), specularPower));

	fragmentColor.xyz = fDirect.xyz * expAtt;
	fragmentColor.w = 0;
}
