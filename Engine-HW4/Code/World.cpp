...
{
	params->fogPlane = fogNode->GetInverseWorldTransform().row2;
	params->fogColor = fogNode->fogColor;
	params->fogParams.x = fogNode->fogDensity;
	params->fogParams.z = Dot(params->fogPlane, cameraNode->GetWorldPosition());
	if (params->fogParams.z < 0.0F)
		params->fogParams.y = 1.0F;
	else
		params->fogParams.y = 0.0F;
	params->fogParams.w = Fsgn(Dot(params->fogPlane, cameraNode->GetWorldPosition()));
