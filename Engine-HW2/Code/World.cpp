...
bool FrustumCamera::SphereVisible(const Point3D& center, float radius) const
{
	...
	for (int i = 0; i < 5; i++) {
		if (Dot(worldFrustumPlane[i], center) <= -radius) {
			return (false);
		}
	}
...
bool FrustumCamera::BoxVisible(const Transform4D& transform, const Vector3D& size) const
{
	...
	Vector3D half = 0.5F * size;
	Point3D center = transform.GetTranslation() + half.x * transform[0] + half.y * transform[1] + half.z * transform[2];
	for (int i = 0; i < 5; i++) {
		float rg = Fabs(Dot(worldFrustumPlane[i], transform[0]) * half.x) + Fabs(Dot(worldFrustumPlane[i], transform[1]) * half.y) + Fabs(Dot(worldFrustumPlane[i], transform[2]) * half.z);
		if (Dot(worldFrustumPlane[i], center) <= -rg) {
			return (false);
		}
	}
	return (true);
...
bool PointLight::SphereIlluminated(const Point3D& center, float radius) const
{
	...
	// The center parameter is the world-space center of the sphere.
	if (Magnitude(center - GetWorldPosition()) >= radius + lightRange)
		return (false);
...
bool PointLight::BoxIlluminated(const Transform4D& transform, const Vector3D& size) const
{
	...
	Vector3D half = 0.5F * size;
	Point3D center = transform.GetTranslation() + half.x * transform[0] + half.y * transform[1] + half.z * transform[2];
	Vector3D v = center - GetWorldPosition();
	float rv = Fabs(Dot(v, transform[0]) / Magnitude(v) * half.x) + Fabs(Dot(v, transform[1]) / Magnitude(v) * half.y) + Fabs(Dot(v, transform[0]) / Magnitude(v) * half.z);
	if (pow(Magnitude(v), 2) >= pow(rv + lightRange, 2))
		return (false); 
